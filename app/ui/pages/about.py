...
CHANGELOG = """V4.6.2
· 消费计划分项新增“完成”勾选：勾选后标记已完成并锁定整行，取消勾选恢复编辑
· 消费计划分项新增“凭证”按钮：可上传、替换、删除并预览消费凭证图片
· 关联流水表格加大行高并自动换行，长备注完整显示
· 记账流水拆分为“交易记录”和“每月强制存款”两个页面，侧边栏展开进入
· 交易记录表格至少显示 20 行
· 工资管理拆分为“工资计算”和“退休金测算”两个页面，侧边栏点击展开/收起
· 消费计划分项可上移/下移排序，新顺序保存后生效，已完成行也可移动
· 完成列与操作列自适应宽度，操作按钮居中显示
· 退休金测算改为全部手填：工作记录新增个人/企业养老缴费比例
· 新增个人养老金测算：可选用年缴存、年化收益与计发月数估算月领金额
· 工资管理专项附加扣除新增个人养老金年度抵扣（上限 12000 元）
· 持仓管理筛选移到“持仓列表”标题栏：类别、账户、资产类型
· 持仓账户列表至少显示 8 行；后台行情刷新不再覆盖正在编辑的单元格
· 推送客户版时屏蔽构建与 Git 命令弹出的终端窗口
· N险N金表格放大字号并至少展示 10 行

V4.6.1
· 修复行情与金价刷新在后台执行，输入文字和快速点击界面不再卡顿
· 全局快捷键：回车保存当前内容，Ctrl+S 保存当前页面，Ctrl+Z 撤销上一步删除
· 消费计划关联流水默认筛选近三个月到今天，表格加大行高并自动换行显示完整
· 圆形问号说明只保留在模块大标题，小标题不再显示问号

V4.6
· 新增消费计划：可创建多个大项计划，如旅行、买相机、养猫等
· 每个计划按分项维护，实际花费 = 关联记账流水 + 手动补录
· 分项可勾选归集记账流水，支持日期范围与商家搜索
· 支持总预算、计划金额对比、删除确认与撤销

V4.5
· 智能报告改为账单报告、工资报告、资产规划建议、综合财务报告，不再生成持仓报告
· 报告按类型引用记账流水、工资管理和资产规划的最新结果
· 开发版升级数据库前自动备份旧库，更新不丢数据
· 全局括号说明改为圆形问号提示，鼠标悬停显示说明

V4.4.1
· 资产总览精简：去掉 12 个月格子，保留总额与“本年度”汇总，卡片放大并加分隔线，年份选择移到本年度上方
· 记账流水新增月份快捷选择：点击某月直接显示该月流水并同步日期范围
· 修复“记支出/记收入”无反应：未选择具体某一天时给出提示，选择单日后新记录加到当天流水末尾
· 记账流水新增“导出 BlueCoins”，按 BlueCoins 中文 CSV 格式导出
· 记账流水新增“每月强制存款”独立维护表，表面是支出、实际计入累计存款
· 账户管理并入持仓管理：先建账户才能添加持仓，持仓渠道只从账户中选择，可按账户筛选
· 持仓代码按回车自动填充名称、资产类型与净值（同花顺接口）
· 新增持仓智能分析问答：日/周/月/季/年报快捷生成 + 自由提问，历史保留并自动压缩上下文，仅回答财务投资话题

V4.4
· 工资管理改为多工资方案标签页：新增后可一直保留，关闭仅收起，重启不丢失
· 新增“历史查询”：已收起的工资方案可随时恢复，也可永久删除
· 资产总览重做：只展示总额与 12 个月格子，点进月份查看明细与强制存款
· 资产总览的收支只统计导入的流水（BlueCoins 等 CSV），投资理财不混入收支
· 修复“理论结余”把房租加回收入的错误，修复有流水时图表数据被覆盖的问题
· 简化主界面，移除重复金额与大表格，界面更清爽
